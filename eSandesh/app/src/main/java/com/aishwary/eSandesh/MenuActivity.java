		...
		_default_profile_pic_child_listener = new ChildEventListener() {
			...
			public void onChildAdded(DataSnapshot _param1, String _param2) {
				...
				if (_childKey.equals("Profile Picture")) {
					default_dp_url = _childValue.get("Profile Picture").toString();
				}
			}
	...
	private void initializeLogic() {
		...
		if (sp.getString("visit", "").equals("")) {
			if (mode.getString("mode", "").equals("register")) {
				...
				if (!(default_dp_url.length() == 0)) {
					userdata.put("profile_picture", default_dp_url);
				}
				else {
					userdata.put("profile_picture", "https://firebasestorage.googleapis.com/v0/b/e-sandesh.appspot.com/o/Profile%20Pictures%2Fenvollpe%202.png-3CMxczMUjEPMAMKZrmc3baCvSmu2?alt=media&token=5fccdad8-0262-493c-a1cf-88363f649532");
				}
