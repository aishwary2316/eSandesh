		...
		_Firebase_auth_sign_in_listener = new OnCompleteListener<AuthResult>() {
			...
			public void onComplete(Task<AuthResult> _param1) {
				...
				if (_success) {
					...
					login.edit().putString("autologin", "allowed").commit();
					login.edit().putString("email", email.getText().toString().trim()).commit();
					login.edit().putString("password", password.getText().toString()).commit();
					intent.setClass(getApplicationContext(), MenuActivity.class);
					startActivity(intent);
					_customText("Successful Sign in");
				}
				else {
					...
					dialog.setPositiveButton("OK", new DialogInterface.OnClickListener() {
						...
						public void onClick(DialogInterface _dialog, int _which) {
							_customText("Sign in Unsuccessful !");
						}
					});
					dialog.create().show();
				}
